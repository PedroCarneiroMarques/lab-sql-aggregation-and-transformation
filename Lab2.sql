...
SELECT 
    DATEDIFF(MAX(rental_date), MIN(rental_date)) AS days_alive
FROM
    sakila.rental;
...
# is NULL, replace it with the string 'Not Available'. Sort the results of the film title in 
# ascending order.
...
SELECT 
    title, IFNULL(rental_duration, 'Not Available') AS available
FROM
    film;
